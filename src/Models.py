...
class Model:
    ...
    def gradient_boosting(self,
        ...
        print("../data/gradient_boosting" +"_" + self.load_file_name + "_round_" + str(self.round) +"_" +\
                     (str(self.pre_start) + "_pre_start_" if self.pre_start!=None else "")+ \
                     (str(self.pre_end) + "_pre_end_" if self.pre_end!=None else "")+ \
                     (str(self.post_start) + "_post_start_" if self.post_start!=None else "")+ \
                     (str(self.post_end) + "_post_end"if self.post_end!=None else "") + \
        ...
        if len(cv_scores['acc']) == 10:
            ...
            # save model to file
            # pickle.dump(model, open("../models/gradient_boosting_" + self.load_file_name + ".pickle.dat", "wb"))
    ...
    def knn_classifier(self,
        ...
        np.save(file="../data/knn" + "_" + self.load_file_name + "_round_" + str(
            self.round) + "_train_prediction.npy", arr=model.predict(self.x_data.argmax(axis=2)[train]))
        np.save(file="../data/knn" + "_" + self.load_file_name + "_round_" + str(
            self.round) + "_prediction.npy", arr=y_pred)

        print("KNN evaluation:", accuracy, precision, recall)

        if len(cv_scores['acc']) == 10:
            print("KNN APPROACH", file=self.filehandler)
            print("Data shape: {}".format(self.x_data.shape), file=self.filehandler)
    ...
    def gaussian_process_classifier(self,
        ...
        np.save(file="../data/gaussian_process" + "_" + self.load_file_name + "_round_" + str(
            self.round) + "_train_prediction.npy", arr=model.predict(self.x_data.argmax(axis=2)[train]))
        np.save(file="../data/gaussian_process" + "_" + self.load_file_name + "_round_" + str(
            self.round) + "_prediction.npy", arr=y_pred)

        print("Gaussian process evaluation:", accuracy, precision, recall)

        if len(cv_scores['acc']) == 10:
            print("GAUSSIAN PROCESS APPROACH", file=self.filehandler)
            print("Data shape: {}".format(self.x_data.shape), file=self.filehandler)
    ...
    def simple_classifier_on_DiProDB(self,
        ...
        convolutional_1_3 = layers.Conv2D(32, kernel_size=(5, 15), activation='relu')(input_tensor)
        ...
        merge_1 = layers.Concatenate(axis=1)([max_pool_1_2, max_pool_1_3])
